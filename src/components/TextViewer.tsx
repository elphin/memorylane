...
import {
  X, Edit, Trash2, Calendar, MapPin, Users, Tag, Folder, Check, ChevronLeft, ChevronRight
} from 'lucide-react'
import { TagInput } from './TagInput'

interface TextViewerProps {
  item: Item
  allItems: Item[]  // All text items in the same event for navigation
  eventTitle: string
...
export function TextViewer({
  ...
  // Filter to only text items for navigation
  const textItems = allItems.filter(i => i.itemType === 'text')
  const currentIndex = textItems.findIndex(i => i.id === item.id)
  const hasPrev = currentIndex > 0
  const hasNext = currentIndex < textItems.length - 1
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowLeft':
          if (hasPrev) {
            onNavigate(textItems[currentIndex - 1])
          }
          break
        case 'ArrowRight':
          if (hasNext) {
  ...
  }, [onClose, hasPrev, hasNext, textItems, currentIndex, onNavigate, isEditing])
  ...
  const handleSave = useCallback(() => {
    ...
    setIsEditing(false)
  }, [item, editContent, editCaption, editDate, editTime, editLocationLabel, editPeople, editTags, editCategory, onSave])
        ...
        <div style={styles.content}>
          {isEditing ? (
            ...
            <div style={styles.textContent}>
              {item.content || <span style={styles.emptyText}>Geen tekst</span>}
            </div>
          )}
        </div>
          ...
          {isEditing ? (
            <div style={styles.editForm}>
              ...
              <div style={styles.formRow}>
                <label style={styles.label}>Onderschrift</label>
                <input
                  type="text"
                  value={editCaption}
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  tag: {
    backgroundColor: 'rgba(93, 122, 160, 0.3)',
    color: '#8ab4f8',
    padding: '2px 10px',
    borderRadius: 12,
    fontSize: 13,
